...
Unity 6 기반 3매치 퍼즐 + TCG 프로젝트다.

## 작업 시작 전에 읽을 것

작업을 이어받는 경우 **[docs/HANDOFF.md](docs/HANDOFF.md)**를 읽는다.
현재 구현 상태, 남은 작업의 선행 순서와 이미 세운 가정이 정리되어 있다.

게임 기획은 **[docs/GDD.md](docs/GDD.md)**가 단일 원천이며 기획자 소유다.
기획에 손댈 일이 생기면 직접 고치지 말고 제안한다. 승인이 확인된 뒤에만 반영한다.
...
### 새 작업은 워크트리가 아니라 브랜치로 시작한다

**작업을 시작할 때는 피처 브랜치를 파고, 로컬 작업 디렉터리도 그 브랜치로 옮긴 뒤에 시작한다.**
에이전트가 별도 워크트리에서 작업하지 않는다.

```bash
git checkout -b feature/<작업이름>
```

워크트리에서 작업하면 **에디터가 여는 경로와 코드가 쓰이는 경로가 갈린다.**
로컬은 `main`을 그대로 보고 있으므로, 수정한 파일이 Unity에 나타나지 않고
"반영이 안 됐다"는 상태가 된다. 실제로 한 번 겪었다.

이 프로젝트에서는 특히 비싸다.

- **컴파일 검증을 사람이 대신한다.** 에이전트는 컴파일 여부를 확인할 수 없어(§0, `CLAUDE.md`)
  에디터에서 즉시 열리는 것이 유일한 검증 경로다. 경로가 갈리면 그 경로가 끊긴다.
- **같은 프로젝트를 두 경로에서 열 수 없다.** `Library` 캐시가 갈려 임포트가 어긋난다.
  워크트리 쪽을 열어 확인하는 우회로도 안전하지 않다.
- **`.meta`와 guid가 두 벌이 된다.** 한쪽에서 에디터가 발급한 `.meta`가 다른 쪽에 없으면
  병합할 때 guid가 갈리고, `.asset`의 스크립트 참조가 끊긴다.
