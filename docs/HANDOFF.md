...
3매치 퍼즐 코어와 **전투 한 판이 승패까지 닫혔고, 화면에도 나온다.**
스왑하면 적 HP가 깎이고, 적도 주기마다 반격하며, 적이 죽으면 승리 · 아군 2인이 죽으면 패배로 끝난다.
비어 있는 것은 **연출의 내용**(모션·스프라이트)과 **스포닝 비율**이다. 구조는 네 층이 다 찼다.
...
| 10 | `T_O` 증폭 액션 + 사망 버블 회색조 | 6 | `IsPreemptive` override 한 줄이면 켜진다 |

**[6] → [10] 순서다.** [10]의 사망 버블 회색조가 [6]의 사망 통지 경로를 그대로 타고,
`T_O` 증폭은 선배치 정렬 규칙을 건드린다.

> **[10] 착수 직전에 `SkillResolver` 분리를 권한다.** `BattleManager.BuildSkillRecipes`
> (매치 그룹 → 스킬 레시피 번역, 선배치 정렬)를 순수 클래스로 뺀다. 참조를 하나도 안 받는
> 순수 함수라 이동 비용이 거의 없고, [10]이 어차피 그 규칙을 고치므로 그때가 자연스럽다.
> 실행(`ExecuteSkill`)과는 **다른 이유로 다른 시점에** 바뀌는 코드다.
...
- [x] 영수증이 화면에서 재생된다 (시전 모션 → 피격 → 게이지)
- [x] **연출이 끝난 뒤 다음 스왑이 된다** — 완수 보고를 `OnComplete`로 옮긴 것이 여기서 걸린다
- [ ] 광역 스킬(`AllAllies`)이 **한 박자**로 나온다. 대상 수만큼 늘어지면 묶기가 안 도는 것이다
- [ ] 적 공격 연출 도중에 스왑해도 진행이 막히지 않는다 (`Complete()` 경로)
- [ ] 연출 종료 시 화면 게이지 == 모델 `CurrentHP` (`ReconcileGauges`)
