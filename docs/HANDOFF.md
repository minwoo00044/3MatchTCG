...
> **[7]·[8]은 에디터에서 검증됐다.** 남은 것은 **승리 경로 하나**다 (적 HP 3000이라 아직 못 봤다).
> 패배·사망·위협도 정렬·실드 흡수·Time Freeze는 전부 로그로 확인했다. 자세한 것은 §1과 §6.
...
- **승리 경로** — 적 HP 3000이라 도달 못 했다. `Enemy Max HP`를 100쯤으로 낮추면 한 번에 확인된다.
  **오버킬 시 연출 완주**(GDD §4.4)가 여기서 유일하게 위험한 지점이다
- **죽은 캐릭터 버블의 스킬 무효화** — 사망 후 스왑을 해봐야 한다
- **위협도 누적분** — 관측한 판에 스왑이 없어 전부 `BaseThreat` 고정값이었다.
  스킬을 쓰면 그 위로 올라갔다가 10초 뒤 내려가야 한다
